...
function ProfileList({ selected, onSelect }: { selected: string; onSelect: (n: string) => void }) {
  ...
  return (
    <>
      ...
      <InfoBox>Profiles with valid = YES are authenticated and ready to use.</InfoBox>
      {profiles.map(p => (
        <PickRow key={p.name} active={selected === p.name} disabled={!p.valid} onClick={() => p.valid && onSelect(p.name)}>
          <Dot color={p.valid ? 'green' : 'gray'} />
          <span className="flex-1 font-mono text-[12px] text-zinc-800 dark:text-zinc-100 truncate">{p.name}</span>
          <span className="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono truncate max-w-[140px]">{p.host.replace('https://', '')}</span>
      ...
      {warehouses.map(wh => {
        ...
        return (
          <PickRow key={wh.id} active={selected === wh.id} onClick={() => onSelect(wh.id, wh.name)}>
            ...
            <span className="flex-1 font-mono text-[12px] text-zinc-800 dark:text-zinc-100">{wh.name}</span>
    ...
    <>
      ...
      {catalogs.map(c => (
        <PickRow key={c} active={catalog === c} onClick={() => onCatalog(c)}>
          <Dot color="green" />
          <span className="font-mono text-[12px] text-zinc-800 dark:text-zinc-100">{c}</span>
        </PickRow>
      ))}
      <div className="mt-3">
    ...
    <>
      ...
      {spaces.map(s => (
        <PickRow key={s.id} active={selected === s.id} onClick={() => onSelect(s.id, s.name)}>
          <Dot color="green" />
          <span className="flex-1 font-mono text-[12px] text-zinc-800 dark:text-zinc-100">{s.name}</span>
          <span className="text-[10px] text-zinc-400 dark:text-zinc-600 font-mono">{s.id.slice(0, 8)}…</span>
        </PickRow>
      ))}
    </>
  )
...
function Label({ children }: { children: React.ReactNode }) {
  return <div className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-2 font-mono">{children}</div>
}

function InfoBox({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded p-2.5 text-[11px] font-mono text-zinc-500 dark:text-zinc-400 leading-relaxed mb-3">
      {children}
    </div>
  )
}
...
function Spinner({ label }: { label: string }) {
  return <div className="text-[11px] text-zinc-400 dark:text-zinc-500 font-mono">{label}</div>
}

function ErrMsg({ msg }: { msg: string }) {
  return <div className="text-[11px] text-red-400 font-mono">{msg}</div>
}
...
function Dot({ color }: { color: 'green' | 'gray' | 'amber' | 'red' }) {
  ...
  return <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${cls[color]}`} />
}
...
function Tag({ color, children }: { color: 'green' | 'purple'; children: React.ReactNode }) {
  ...
  return <span className={`text-[10px] rounded px-1.5 py-0.5 font-mono ${cls}`}>{children}</span>
}
...
function PickRow({ active, disabled = false, onClick, children }: {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`
        w-full flex items-center gap-2 px-2.5 py-2 rounded border mb-1.5 text-left transition-colors
        ${active
...
function Input({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <input
      ...
      className="w-full text-[12px] font-mono bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded px-2.5 py-1.5 outline-none focus:border-[#534AB7] dark:focus:border-[#7b74e8] text-zinc-800 dark:text-zinc-100 placeholder:text-zinc-300 dark:placeholder:text-zinc-600"
    />
  )
}
...
function Terminal({ lines }: { lines: ExecLine[] }) {
  ...
  return (
    <div ref={ref} className="bg-[#0d0d10] rounded p-3 font-mono text-[11px] leading-relaxed h-[280px] overflow-y-auto border border-zinc-800/50">
...
export function SetupDrawer({
  activeStep, phase, selectedChoice, execLines,
  onSelectChoice, onContinue, onBack, onReconfigure, onExecDone,
}: SetupDrawerProps) {
      ...
      <>
        <div className="flex-1 overflow-y-auto px-4 pt-3 pb-2">
          {step.choices.map((c, i) => (
            <button
              ...
              className={`
                w-full flex items-start gap-2.5 px-3 py-2.5 rounded border mb-2 text-left transition-colors
                ${selectedChoice === i
                  ? 'border-[#534AB7] dark:border-[#7b74e8] bg-[#EEEDFE] dark:bg-[#2d2960]/50'
              ...
              `}
            >
              <span className={`text-[11px] font-mono mt-0.5 min-w-[14px] ${selectedChoice === i ? 'text-[#534AB7] dark:text-[#9f9af5]' : 'text-zinc-400 dark:text-zinc-500'}`}>
                {i + 1}
              </span>
              <div>
                <div className="text-[12px] font-medium text-zinc-800 dark:text-zinc-100 leading-tight">{c.title}</div>
                <div className="text-[11px] text-zinc-400 dark:text-zinc-500 leading-snug mt-0.5">{c.desc}</div>
              </div>
            </button>
          ))}
        </div>
        <div className="px-4 py-3 border-t border-zinc-100 dark:border-zinc-800">
          <button
            ...
            className={`
              w-full text-[12px] py-2 rounded border font-mono transition-colors
              ${selectedChoice !== null
  ...
  function renderConfigure() {
    ...
    if (action === 'cfg-profile')
      ...
      body = (<><Label>value</Label><Input value={manualVal} onChange={setManualVal} placeholder="paste value…" /><div className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-2 font-mono">writes directly to .env.local</div></>)
    ...
    return (
      <>
        ...
        <div className="px-4 py-3 border-t border-zinc-100 dark:border-zinc-800 flex flex-col gap-1.5">
          ...
          <button
            onClick={onBack}
            className="w-full text-[12px] py-1.5 rounded border border-zinc-200 dark:border-zinc-800 text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 hover:border-zinc-300 dark:hover:border-zinc-700 font-mono transition-colors"
          >
            back
          </button>
        </div>
  ...
  function renderExecute() {
    return (
      <>
        ...
        <div className="px-4 py-3 border-t border-zinc-100 dark:border-zinc-800">
          <button disabled className="w-full text-[12px] py-2 rounded border border-zinc-200 dark:border-zinc-800 text-zinc-300 dark:text-zinc-600 cursor-not-allowed font-mono">
            running…
          </button>
        </div>
      </>
    ...
    return (
      <>
        <div className="flex-1 flex flex-col items-center justify-center px-4 pt-4 pb-2">
          <div className="text-3xl text-[#1D9E75] mb-2">✓</div>
          <div className="text-[13px] font-medium text-zinc-800 dark:text-zinc-100 mb-1">configured</div>
          <div className="text-[11px] text-zinc-400 dark:text-zinc-500 font-mono">{step.title}</div>
        </div>
        <div className="px-4 py-3 border-t border-zinc-100 dark:border-zinc-800">
          <button
            onClick={onReconfigure}
            className="w-full text-[12px] py-2 rounded border border-[#534AB7] dark:border-[#7b74e8] text-[#534AB7] dark:text-[#9f9af5] hover:bg-[#EEEDFE] dark:hover:bg-[#2d2960]/50 font-mono transition-colors"
          >
            reconfigure
          </button>
        </div>
    ...
    <div className="flex flex-col h-full bg-white dark:bg-zinc-900">
      ...
      <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 flex-shrink-0">
        <div className="flex items-center justify-between mb-0.5">
          <div className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-wider font-mono">{step.label}</div>
          {phase !== 'choose' && (
            <button
              onClick={onBack}
              className="text-[11px] text-zinc-400 dark:text-zinc-500 border border-zinc-200 dark:border-zinc-700 rounded px-1.5 py-0.5 hover:text-zinc-600 dark:hover:text-zinc-300 font-mono transition-colors"
            >
              ← back
            </button>
          )}
        </div>
        <div className="text-[13px] font-medium text-zinc-800 dark:text-zinc-100 font-mono">{step.title}</div>
      </div>
      ...
      <div className="flex items-center px-4 py-2 border-b border-zinc-100 dark:border-zinc-800 flex-shrink-0">
        {PHASES.map((p, i) => (
          <span key={p} className="flex items-center">
            ...
            {i < PHASES.length - 1 && <span className="text-[10px] text-zinc-200 dark:text-zinc-700 mx-1.5">›</span>}
          </span>
        ))}
      </div>
